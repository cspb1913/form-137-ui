...
/**
 * Retrieve Auth0 access token from the frontend API endpoint
 * 
 * @returns Promise<string> - Auth0 access token
 */
export async function getAuth0AccessToken(): Promise<string> {
  try {
    console.log('🔐 Fetching Auth0 access token...')
    const response = await fetch('/api/auth/access-token')
    
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}))
      throw new Error(errorData.message || `Token request failed: ${response.status} ${response.statusText}`)
    }
    ...
    console.error("Failed to retrieve Auth0 access token:", error)
    throw new Error("Auth0 authentication failed. Please log in again.")
  }
}
